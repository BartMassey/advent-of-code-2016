...
/// set of items.
pub fn choose<T>(source: &BTreeSet<T>, n: usize) -> BTreeSet<BTreeSet<T>>
where
    T: Clone + Ord,
{
    if source.len() < n {
...
/// set of items.
pub fn choose_le<T>(source: &BTreeSet<T>, n: usize) -> BTreeSet<BTreeSet<T>>
where
    T: Clone + Ord,
{
...
mod tests {
    ...
    fn four_choose_two() {
        ...
        let cs = choose(&s, 2);
        let t = make_set_set(&[&[1, 2], &[1, 3], &[1, 4], &[2, 3], &[2, 4], &[3, 4]]);
        assert!(cs == t);
    }
}
