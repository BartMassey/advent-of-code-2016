...
lazy_static! {
    static ref VALUE_PAT: regex::Regex = regex::Regex::new(r"^value (\d+) goes to (.+)$")
    ...
    static ref BOT_PAT: regex::Regex =
        regex::Regex::new(r"^bot (\d+)$").expect("could not compile regex for bot");
    static ref OUTPUT_PAT: regex::Regex =
        regex::Regex::new(r"^output (\d+)$").expect("could not compile regex for output");
}
...
pub fn main() {
    ...
    // Set up parameters.
    let targets = if part == 1 {
        assert!(target.len() == 2);
        let target_low = target[0].parse().expect("main: could not parse low target");
        let target_high = target[1]
            .parse()
    ...
    for l in aoc::input_lines() {
        ...
        match (*self::VALUE_PAT).captures(&l) {
            ...
            Some(parts) => {
                ...
                let value_part = parts
                    ...
                    .parse()
                    .expect("main: could not parse value");
                let dest_part = parse_dest(parts.at(2).expect("main: could not find dest"));
                match dest_part {
                    Bot(b) => {
                        max_bot = max(b, max_bot);
                    }
                    Output(q) => {
                        max_output = max(q, max_output);
                    }
                };
        ...
        match (*self::COMPARE_PAT).captures(&l) {
            ...
            Some(parts) => {
                ...
                let bot_part = parts
                    .at(1)
                    .expect("main: could not find bot")
                    .parse()
                    .expect("main: could not parse bot");
                ...
                insns.push(Compare(bot_part, dest_low_part, dest_high_part));
                continue;
            }
        };
            ...
            for vl in &holds[b].clone() {
                for vh in &holds[b].clone() {
                    ...
                    if let Some((target_low, target_high)) = targets {
                        if holds[b].contains(&target_low) && holds[b].contains(&target_high) {
                            println!("{}", b);
                            return;
                        }
                    };
                    ...
                    for i in &insns {
                        if let Compare(b0, ref dl, ref dh) = *i {
                            if b == b0 {
                                ...
                                match *dh {
                                    Bot(bh) => {
                                        if holds[bh].len() < 2 && holds[bh].insert(*vh) {
                                            changed = true;
                                            if TRACE {
                                                println!(
                                                    "{} high {} to {}({})",
                                    ...
                                    Output(qh) => {
                                        ...
                                        if !unchanged {
                                            ...
                                            changed = true;
                                            if TRACE {
                                                println!("{} high {} to output {}", b, *vh, qh);
                                            }
                                        }
                                    }
                                };
